...
if (empty($_POST['identifier'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if (!empty($bitcoind->wallet($_SESSION['user'])->listtransactions())) {
                $transactions = $bitcoind->wallet($_SESSION['user'])->listtransactions();
...
if(isset($_POST['order_review_form_btn'])){
    ...
    $vendor_feedback    =   $_POST['vendor_feedback'];
    ...
    echo 'Vendor feedback:'.$vendor_feedback;
...
VALUES ("' . $_SESSION['user'] . '", "' . $vendor . '",'.$pid.','.$items_delivered.' , "' . $review_title . '", "' . $review_description . '",'.$user_rating.', "'.$rated_on.'")';
    ...
    //insert user review feedback
    //insert review
    $sql = 'INSERT INTO `user_feedbacks`
    ( user_id, vendor_id, feedback_value )
VALUES ("' . $_SESSION['user'] . '", "' . $vendor . '",'.$vendor_feedback.'")';
    if (!mysqli_query($conn, $sql)) {
        echo '<br>Cannot save User Review Feedback.';
        echo mysqli_error($conn);
    } else {
        echo '<br>Created User Review Feedback.';
    }
...
margin-bottom: 15px;">Order Received?</h3>
                        ...
                        <label>Give feedback to the vendor:</label>
                        <select id="vendor_feedback" name="vendor_feedback" class="form-control">
